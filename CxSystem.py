...
class CxSystem(object):
    ...
    def set_runtime_parameters(self):
        ...
        if self.do_benchmark:
            print(" -  CxSystem is performing benchmarking. The Brian2 should be configured to use benchmarking.")
        if self.device.lower() == 'genn':
...
#            prefs.codegen.cpp.extra_compile_args_gcc = ['-O3', '-pipe']
    ...
    def _set_grid_radius(self, *args):
        ...
        try:
            if self.scale!=1 :
                print(" -  Radius of the system scaled to %s from %s" % (str(sqrt(self.scale)*self.general_grid_radius), str(self.general_grid_radius)))
            self.general_grid_radius = sqrt(self.scale)*self.general_grid_radius
            if self.sys_mode != 'expanded' and self.scale != 1:
                print(' -  System is scaled by factor of %f but the system mode is local instead of expanded'%(self.scale))
        except AttributeError:
            pass
    ...
    def _set_load_brian_data_path(self, *args):
        ...
        if 'gz' in self.load_brian_data_extension:
            ...
            with open(self.load_brian_data_path, 'rb') as fb:
                self.loaded_brian_data = pickle.load(fb)
        print(' -  Brian data file loaded from %s'%os.path.abspath(self.load_brian_data_path))
        ...
        for syn in self.current_values_list:
            ...
            if self.device.lower() == 'python' :
                ...
                try:
                    ...
                    print(" -  number of synapses for last connection was equal to number of connections")
        ...
        * number_of_neurons: The number of neurons that exist in the input .mat file.
        ...
        def video(self):
            ...
            def waitress(self):
                ...
                exec("%s=%s" % (thread_NN_name, thread_number_of_neurons), globals(), locals())
                exec("%s=%s" % (thread_NE_name, Eq), globals(), locals())
                exec("%s=%s" % (thread_NT_name, "'emit_spike>=1'"), globals(), locals())
                exec("%s=%s" % (thread_NRes_name, "'emit_spike=0'"), globals(), locals())
                exec("%s= NeuronGroup(%s, model=%s,method='%s', threshold=%s, reset=%s)" \
                     % (thread_NG_name, thread_NN_name, thread_NE_name, self.numerical_integration_method,thread_NT_name, thread_NRes_name), globals(), locals())
                if hasattr(self, 'loaded_brian_data'):
                ...
                exec("%s.x=real(self.customized_neurons_list[%d]['w_positions'])*mm\n" \
                     "%s.y=imag(self.customized_neurons_list[%d]['w_positions'])*mm" % \
                     (thread_NG_name, self.video_input_idx, thread_NG_name, self.video_input_idx), globals(), locals())
                ...
                exec("%s = Synapses(SPK_GENERATOR, %s, on_pre='emit_spike+=1')" % \
                     (thread_SGsyn_name, thread_NG_name), globals(), locals())# connecting the SpikeGeneratorGroup() and relay group.
                exec("%s.connect(j='i')" % thread_SGsyn_name, globals(), locals()) # SV change
...
#            exec('spike_times_ = spike_times_list * %s' %(spike_times_unit), globals(), locals())
            ...
            exec(spikes_str, globals(), locals())  # running the string containing the syntax for Spike indices in the input neuron group.
            exec(times_str, globals(), locals())  # running the string containing the syntax for time indices in the input neuron group.
            exec(SG_str, globals(), locals())  # running the string containing the syntax for creating the SpikeGeneratorGroup() based on the input .mat file.
            ...
            Eq = """'''emit_spike : 1
                            x : meter
                            y : meter'''"""
            exec("%s=%s" % (_dyn_neuronnumber_name, number_of_neurons), globals(), locals())
            exec("%s=%s" % (_dyn_neuron_eq_name, Eq), globals(), locals())
            exec("%s=%s" % (_dyn_neuron_thres_name, "'emit_spike>=1'"), globals(), locals())
            exec("%s=%s" % (_dyn_neuron_reset_name, "'emit_spike=0'"), globals(), locals())
            exec("%s= NeuronGroup(%s, model=%s, method='%s',threshold=%s, reset=%s)" \
                 % (_dyn_neurongroup_name, _dyn_neuronnumber_name, _dyn_neuron_eq_name, self.numerical_integration_method, _dyn_neuron_thres_name, _dyn_neuron_reset_name), globals(), locals())
            ...
            exec("%s.x=real(self.customized_neurons_list[%d]['w_positions'])*mm\n"\
                 "%s.y=imag(self.customized_neurons_list[%d]['w_positions'])*mm" \
                 %(_dyn_neurongroup_name, current_idx, _dyn_neurongroup_name, current_idx), globals(), locals())
            ...
            exec("%s = Synapses(GEN, %s, on_pre='emit_spike+=1')" \
                 % (SGsyn_name, _dyn_neurongroup_name), globals(), locals())  # connecting the SpikeGeneratorGroup() and relay group.
        ...
        def spikes(self):
            ...
            print(" -   Spike file loaded from: %s" %os.path.join(self.output_folder, input_spikes_filename))
            ...
            exec("%s=%s" % (NRes_name, "'emit_spike=0'"), globals(), locals())
            exec("%s= NeuronGroup(%s, model=%s,method='%s', threshold=%s, reset=%s)" \
            ...
            exec("%s.x=real(self.customized_neurons_list[%d]['w_positions'])*mm\n" \
                 "%s.y=imag(self.customized_neurons_list[%d]['w_positions'])*mm" % \
                 (NG_name, self.spike_input_group_idx, NG_name, self.spike_input_group_idx), globals(), locals())
            ...
            exec("%s = Synapses(SPK_GENERATOR, %s, on_pre='emit_spike+=1')" % \
                 (SGsyn_name,
                  NG_name), globals(), locals())  # connecting the SpikeGeneratorGroup() and relay group.
            exec("%s.connect(j='i')" % SGsyn_name, globals(), locals())  # SV change
...
if __name__ == '__main__' :
    try: # this runs in cluster
        net_config = sys.argv[1]
        phys_config = sys.argv[2]
        try:
            ar_run_start_idx = sys.argv[3]
            ar_run_step = sys.argv[4]
            CM = CxSystem(net_config, phys_config, cluster_run_start_idx=ar_run_start_idx, cluster_run_step=ar_run_step)
        except IndexError:
