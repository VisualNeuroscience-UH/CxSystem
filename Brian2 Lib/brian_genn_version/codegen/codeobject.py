...
from brian_genn_version.equations.unitcheck import check_units_statements
from brian_genn_version.utils.logger import get_logger
from brian_genn_version.utils.stringtools import indent, code_representation
...
def constant_or_scalar(varname, variable):
    ...
    '''
    from brian_genn_version.devices.device import get_device  # avoid circular import
    if variable.array:
...
class CodeObject(Nameable):
    ...
    `brian_genn_version.codegen.templates.MultiTemplate`.
...
def create_runner_codeobj(group, code, template_name,
    ...
    logger.debug(msg)
    from brian_genn_version.devices import get_device
    device = get_device()
