...
def brianobj_role(role, rawtext, text, lineno, inliner, options={}, content=[]):
    ...
    a `from brian_genn_version import *`, e.g `NeuronGroup`.
    ...
    if text in prefs:
        ...
        if text and (not '~' in text):
            try:
                ...
                if not '.' in text:
                    module = __import__('brian_genn_version', fromlist=[str(text)])
                ...
                # Possibly a method/classmethod/attribute name
                    ...
                    if attrname.endswith('()'):
                        attrname = attrname[:-2]
                    module = __import__('brian_genn_version', fromlist=[str(classname)])
