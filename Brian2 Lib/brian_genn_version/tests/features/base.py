...
from brian_genn_version.utils.stringtools import indent
...
class BaseTest(object):
    ...
    def timed_run(self, duration):
        ...
        '''
        if isinstance(brian_genn_version.get_device(), brian_genn_version.devices.RuntimeDevice):
            brian_genn_version.run(brian_genn_version.defaultclock.dt, level=1)
            brian_genn_version.run(duration, level=1)
        else:
            brian_genn_version.run(duration, level=1)
...
class Configuration(object):
    ...
    def __init__(self, maximum_run_time=1e7*brian_genn_version.second):
        maximum_run_time = float(maximum_run_time)*brian_genn_version.second
    ...
    def get_last_run_time(self):
        ...
        '''
        if hasattr(brian_genn_version.device, '_last_run_time'):
            return brian_genn_version.device._last_run_time
        raise NotImplementedError
    ...
    def get_last_run_completed_fraction(self):
        ...
        '''
        if hasattr(brian_genn_version.device, '_last_run_completed_fraction'):
            return brian_genn_version.device._last_run_completed_fraction
        return 1.0
...
class DefaultConfiguration(Configuration):
    ...
    def before_run(self):
        ...
        brian_genn_version.set_device('runtime')
...
class LocalConfiguration(Configuration):
    ...
    def before_run(self):
        brian_genn_version.prefs.reset_to_defaults()
        brian_genn_version.set_device('runtime')
        brian_genn_version.prefs.load_preferences()
...
class NumpyConfiguration(Configuration):
    ...
    def before_run(self):
        brian_genn_version.prefs.reset_to_defaults()
        brian_genn_version.set_device('runtime')
        brian_genn_version.prefs.codegen.target = 'numpy'
...
class WeaveConfiguration(Configuration):
    ...
    def before_run(self):
        brian_genn_version.prefs.reset_to_defaults()
        brian_genn_version.set_device('runtime')
        brian_genn_version.prefs.codegen.target = 'weave'
...
class CythonConfiguration(Configuration):
    ...
    def before_run(self):
        brian_genn_version.prefs.reset_to_defaults()
        brian_genn_version.set_device('runtime')
        brian_genn_version.prefs.codegen.target = 'cython'
...
class CPPStandaloneConfiguration(Configuration):
    ...
    def before_run(self):
        brian_genn_version.prefs.reset_to_defaults()
        brian_genn_version.set_device('cpp_standalone')
        
    def after_run(self):
        if os.path.exists('cpp_standalone'):
            shutil.rmtree('cpp_standalone')
        brian_genn_version.device.build(directory='cpp_standalone', compile=True, run=True,
                            with_output=False)
...
class CPPStandaloneConfigurationOpenMP(Configuration):
    ...
    def before_run(self):
        brian_genn_version.prefs.reset_to_defaults()
        brian_genn_version.set_device('cpp_standalone')
        brian_genn_version.prefs.devices.cpp_standalone.openmp_threads = 4
...
def results(configuration, feature, n=None, maximum_run_time=1e7*brian_genn_version.second):
...
try:
    ...
    brian_genn_version.device._set_maximum_run_time({maximum_run_time})
    feature.run()
...
def run_feature_tests(configurations=None, feature_tests=None,
                      strict=1e-5, tolerant=0.05, verbose=True, maximum_run_time=1e7*brian_genn_version.second):
    if configurations is None:
        # some configurations to attempt to import
        try:
            import brian_genn_versiongenn.correctness_testing
        except:
            pass
...
def run_speed_tests(configurations=None, speed_tests=None, run_twice=True, verbose=True,
                    n_slice=slice(None), maximum_run_time=1e7*brian_genn_version.second):
    if configurations is None:
        # some configurations to attempt to import
        try:
            import brian_genn_versiongenn.correctness_testing
