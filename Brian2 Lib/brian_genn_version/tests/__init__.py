...
from brian_genn_version.devices.device import all_devices
...
def run(codegen_targets=None, long_tests=False, test_codegen_independent=True,
    ...
    try:
        ...
        for target in codegen_targets:
            ...
            if not long_tests:
                exclude_str += ',!long'
            # explicitly ignore the brian_genn_version.hears file for testing, otherwise the
            ...
            success.append(nose.run(argv=argv))
        if test_standalone:
            from brian_genn_version.devices.device import get_device, set_device
