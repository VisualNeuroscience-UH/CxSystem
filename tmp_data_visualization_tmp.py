...
time_for_visualization=np.array([17,18])
...
# create time vector for vm x-axis
time_vector_for_plot = np.arange(time_for_visualization[0],time_for_visualization[1], dt/(1 * second))
...
for plot_index, neuron_group in enumerate(neuron_groups):
    ...
    if neuron_group in vm_all.keys():
        ...
        plt.plot(np.arange(time_for_visualization[0],time_for_visualization[1],dt / ms),membrane_voltages[:,time_vector_for_plot].T, '-')
