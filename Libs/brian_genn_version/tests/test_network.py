...
from brian_genn_version import (Clock, Network, ms, second, BrianObject, defaultclock,
...
from brian_genn_version.devices.device import restore_device, Device, all_devices, set_device, get_device
...
def test_magic_weak_reference():
    ...
    with catch_logs(log_level=logging.DEBUG) as l:
        ...
        magic_objects = [msg[2] for msg in l
                         if msg[1] == 'brian_genn_version.core.magic.magic_objects'][0]
    ...
    for obj in dependent_objects:  # obj is our explicit reference
        with catch_logs() as l:
            ...
            dependency_warnings = [msg[2] for msg in l
                                   if msg[1] == 'brian_genn_version.core.magic.dependency_warning']
