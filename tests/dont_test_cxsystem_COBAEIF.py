...
class TestConfigurationExecutor:
	...
	def test_relay(self):
		...
		assert list(CM.customized_neurons_list[0].keys()) == [
			'z_positions', 'w_positions', 'equation', 'type', 'idx']
		assert list(CM.customized_neurons_list[1].keys()) == [
		...
		assert list(CM.customized_neurons_list[2].keys()) == [
			'reset', 'w_positions', 'total_comp_num', 'soma_layer', 
			'idx', 'dends_layer', 'z_positions', 'equation', 
			'namespace', 'refractory', 'object_name', 'dend_comp_num', 
			'w_center', 'threshold', 'number_of_neurons', 'type', 'z_center']
...
class TestPhysiologyReference:
	...
	def test_neuron_reference_init(self):
		assert CM.customized_neurons_list[0]['z_positions'] == [np.e ** (x/17) - 1 for x in CM.customized_neurons_list[0]['w_positions']]
		assert CM.customized_neurons_list[1]['z_positions'] == [np.e ** (x/17) - 1 for x in CM.customized_neurons_list[1]['w_positions']]
		assert CM.customized_neurons_list[2]['z_positions'] == [np.e ** (x/17) - 1 for x in CM.customized_neurons_list[2]['w_positions']]
