...
preprocessed_markram_anatomy_json = 'kukko.json'
...
def plot_local_connectivity(local_connectivity_json=cxs_local_connectivity_json):
    ...
    plt.tight_layout()
    plt.savefig(plot_directory + 'cxs_local_connectivity' + '.eps', dpi=600)
...
def plot_markram_connectivity(markram_json = preprocessed_markram_anatomy_json):
    ...
    plotobj = sns.heatmap(plotdata, cmap=mycmap, vmin=0, vmax=4.0)
    ...
    cbar.set_label('Connection probability')
    plt.tight_layout()
    plt.savefig(plot_directory + 'markram_local_connectivity' + '.eps', dpi=600)
